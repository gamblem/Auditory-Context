...
% Tone).  This is followed by 10 tones. Half of the trials are One-Deviant
% only, the other half are Two-Deviant trials. 

% In the One-Deviant Trials, 9 out of 10 tones are the same, one differs. In
% the Two-Deviant Trials, 8 out of the 10 tones are the same, 1-high tone,
% 1-low tone.
...
A=1; %Aud Amplitude
env=8; % duration of sound ramp up and down= in ms
...
condMatrixBase = [sort(repmat([1 2], 1, 4)) sort(repmat([1],1,4)) sort(repmat([2],1,4));repmat([1 2 3 4], 1, 2) (repmat([5 7 5 7], 1, 1)) (repmat([6 8 6 8], 1, 1))];
...
%SecondRow=Trial type  Single Dev= 1= low tone,pure, 2=High tone,pure, 3=Low,amp 4=High, amp,
%    Dual Deviant 5=Low tone, pure, 6- High tone, pure, 7= low, amp 8= high,amp
...
% Set the coordinates (these are all relative to zero)
...
Devpos=randi([6,10],2,numTrials);

% Second deviant position
for j=1:length(Devpos)
    while Devpos(1,j)==Devpos(2,j)
        Devpos(2,j)=randi([6,10]);
    end
end

%Inter-Stimulus Interval between auditory stimuli per trial
...
for j=1:numTrials
    ...
    elseif TrialMatrixShuffled(2,j) == 1
        ...
        AudStream(j,Devpos(1,j))=1;
        
    elseif TrialMatrixShuffled(2,j) == 2
        AudStream(j,:,1)=[9 9 9 9 9 9 9 9 9 9];
        % Deviant Tone is a Hight tone, pure
        AudStream(j,Devpos(1,j))=2;
    elseif TrialMatrixShuffled(2,j)== 3
        AudStream(j,:,1)=[9 9 9 9 9 9 9 9 9 9];
        % Deviant Tone is a low tone, amp
        AudStream(j,Devpos(1,j))=3;
    elseif TrialMatrixShuffled(2,j)==4
        AudStream(j,:,1)=[9 9 9 9 9 9 9 9 9 9];
        % Deviant Tone is a high tone, amp
        AudStream(j,Devpos(1,j))=4;
    elseif TrialMatrixShuffled(2,j)==5 % Two Deviant Trials
        AudStream(j,:,1)=[9 9 9 9 9 9 9 9 9 9];
        % Deviant Tone is a high tone, amp
        AudStream(j,Devpos(1,j))=5;
        AudStream(j,Devpos(2,j))= randsample([6 8],1); % nontarget Stimulus
    elseif TrialMatrixShuffled(2,j)==6 
        AudStream(j,:,1)=[9 9 9 9 9 9 9 9 9 9];
        % Deviant Tone is a high tone, amp
        AudStream(j,Devpos(1,j))=6;
        AudStream(j,Devpos(2,j))= randsample([5 7],1);
    elseif TrialMatrixShuffled(2,j)==7
                AudStream(j,:,1)=[9 9 9 9 9 9 9 9 9 9];
        % Deviant Tone is a high tone, amp
        AudStream(j,Devpos(1,j))=7;
        AudStream(j,Devpos(2,j))= randsample([6 8],1);
    elseif TrialMatrixShuffled(2,j)==8
        AudStream(j,:,1)=[9 9 9 9 9 9 9 9 9 9];
        % Deviant Tone is a high tone, amp
        AudStream(j,Devpos(1,j))=8;
        AudStream(j,Devpos(2,j))= randsample([5 7],1);
...
for j=1:numTrials
    ...
    for k=1:10
        ...
        elseif AudStream(j,k,1)==5 % Create Low Pure Tone- 2 Dev
              ToneStream(:,k)=makeTone(LowPA,AudDur,500,env,1, myTDT.sampleRate);
            EventCode(j,k)=LowPure2Dev+k;
        elseif AudStream(j,k,1)==6 % Create High Pure Tone- 2 Dev
            ToneStream(:,k)=makeTone(HighPA,AudDur,3000,env,1, myTDT.sampleRate);
            EventCode(j,k)=HighPure2Dev+k;
        elseif AudStream(j,k,1)==7 % Create Low Amp Tone- 2 Dev
             ToneStream(:,k)=makeTone(LowAA,AudDur,500,env,2, myTDT.sampleRate);
            EventCode(j,k)=LowAmp2Dev+k;
        elseif AudStream(j,k,1)==8 % Create High Amp Tone- 2 Dev
            ToneStream(:,k)=makeTone(HighAA,AudDur,3000,env,2, myTDT.sampleRate);
            EventCode(j,k)=HighAmp2Dev+k;  
        end
...
%         
    ...
    if strcmp(color,'red')==1
        Screen('DrawLines', window, allCoords, lineWidthPix, black, [xCenter yCenter], 2); % Fixation cross won't actually change color
...
%    WaitSecs(0.5);
   % KbEventFlush;
        ...
        q=1;
        LBCount=1;
     %Insert Start recording code HERE
    else
    end
...
save(([writepath 'Behavioral_Data\' subID '\' subID '_bdata_MX50.mat']), '-struct', 'PresentationData' );
